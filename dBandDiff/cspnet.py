...
from dBandDiff.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPNet(nn.Module):
    ...
    def forward(self, t, wyckoff_ops, d_band_center, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        #spg_number_per_atom = spg_number[node2graph]
        d_band_center_per_atom = d_band_center[node2graph]
        
        t_per_atom = t.repeat_interleave(num_atoms, dim=0)
        
        '''
        print(f'node_features的形状是{node_features.shape}')    
        print(f'wyckoff_ops的形状是{wyckoff_ops.shape}')
        print(f'd_band_center_per_atom的形状是{d_band_center_per_atom.shape}')
        print(f't的形状是{t.shape}')   
        print(f't_per_atom的形状是{t_per_atom.shape}')
        '''
        
        #con_per_atom = spg_number_per_atom + d_band_center_per_atom + t_per_atom
        node_features = torch.cat([node_features, d_band_center_per_atom, t_per_atom], dim=1)

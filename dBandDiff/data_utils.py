...
def build_crystal(crystal_str, niggli=False, primitive=True):
    """Build crystal from cif string."""
    crystal = Structure.from_str(crystal_str, fmt='cif')
    #crystal = Structure.from_file(crystal_path)
    if primitive:
    ...
    canonical_crystal = Structure(
        lattice=Lattice.from_parameters(*crystal.lattice.parameters), #equal to 'Lattice.from_parameters(a, b, c, alpha, beta, gamma)'
...
def process_one(structure_folder, row, niggli, primitive, graph_method, prop_list, use_space_group = False, tol=0.01):
    crystal_str = row['cif']
    ...
    if use_space_group:
        ...
        result_dict['spacegroup'] = row['spg_number']
        
    result_dict['mp_id'] = row['mp_id']
    
    graph_arrays = build_crystal_graph(crystal, graph_method)
    #result_dict['d_band_center'] = row['d_band_center']
    result_dict['d_band_center'] = round(row['d_band_center'], 3)
    result_dict.update({
...
def preprocess(input_file, structure_folder, num_workers, niggli, primitive, graph_method,
               prop_list, use_space_group = False, tol=0.01):
    df = pd.read_csv(input_file)
    #df = df_origin[df_origin['num_atoms'] <= 20]

    unordered_results = p_umap(
        process_one,
        [structure_folder] * len(df),
    ...
    ordered_results = [mpid_to_results[df.iloc[idx]['mp_id']]
